...
$(function(){
    ...
    $("#table_list_wechatAndAlipay").jqGrid({
    	caption: "自助终端自助建档",
    	url:"pages/WechatAndAlipay/findWechatAndAlipayByPage.action",
        mtype:"POST",
        datatype: "json",
        height: "100%",
        autowidth: true,
        ...
        autowidth: true,
        multiselect: true,
        colNames: ['医院机构','支付方式','金额', '支付时间','支付状态','患者ID','身份证号','姓名','订单号','外部订单编号'],
        colModel: [
            {
                name: 'yyjgId',
                index: 'yyjgId',
...
//            }
        ...
        //treeGrid:true,
        pager: "#pager_list_wechatAndAlipay",
        jsonReader : {   
            root: "data",
            total: "total",
            records: "records"
        ...
        onSelectRow:function(id){
        	...
        	if(id && id!==lastSel){
        		 $("#table_list_wechatAndAlipay").restoreRow(lastSel);
        		 lastSel=id;
        	}
        	$("#table_list_wechatAndAlipay").editRow(id,true);
        },
    ...
    // Add selection
    $("#table_list_wechatAndAlipay").setSelection(4, true);


    // Setup buttons
    $("#table_list_wechatAndAlipay").jqGrid('navGrid', '#pager_list_wechatAndAlipay', {
        edit: false,
        add: false,
        del: false,
        search: false
    ...
    $(window).bind('resize', function () {
        ...
        $('#table_list_wechatAndAlipay').setGridWidth(width);
    });
    //为表单设置样式
...
function edit_perssion_action(){
	var ids = $("#table_list_wechatAndAlipay").jqGrid("getGridParam", "selarrrow")
	 if (ids.length==0) {
		  clearModelId();
	      $.alert("请选择一条数据");
	      return;
		 ...
		 var id = jQuery("#table_list_wechatAndAlipay").jqGrid('getGridParam', 'selrow');
    	 var row = jQuery("#table_list_wechatAndAlipay").jqGrid('getRowData', id);
    	 console.log(row);
...
function edit_perssion(){
	if($("#form1").valid()==false){
     ...
     $.post(url,param,function(result){
    	 if(result.state==SUCCESS){
 			$.alert("修改成功");
 			$("#table_list_wechatAndAlipay").trigger("reloadGrid");
 		}else{
 			$.alert(result.message);
 		}
     });
...
function deleteAll(){
	...
	for(var i=0;i<ids.length;i++){
		//选中行的时间
		var action_id=$("#table_list_wechatAndAlipay").getCell(ids[i],"action_id");
		ids_str+=action_id+":";
	}
...
function likeSerch(){
	...
	var search_starttime=$("#search_starttime").val();
	var search_endtime=$("#search_endtime").val();
	var url="pages/wechatAndAlipay/findWechatAndAlipayByPage.action";
...
//		      };
	var param={
			...
			$("#table_list_wechatAndAlipay").jqGrid('setGridParam',{
					url:url,
					datatype:'json',
					postData:param,
					pager: "#pager_list_wechatAndAlipay",
